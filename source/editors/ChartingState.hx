...
class ChartingState extends MusicBeatState
{
	...
	var eventStuff:Array<Dynamic> = [
		...
		[
			'Fade Strumlines',
			"Makes the gray strum notes fade out until invisible.\nAlso makes singing/combat notes invisible.\n\nValue 1: Character to change (Dad, BF, GF)\nDefaults to enemy/dad strums if empty.\n\nValue 2: Type \"unfade\" to return note visibility."
		] // End of changes
	];
	...
	**/
	...
	// Combat change
	var check_isUnblockable:FlxUICheckBox;
	...
	override function create()
	{
		...
		// Combat change
		curRenderedCombatNoteBools = new FlxTypedGroup<FlxSprite>();
		...
		// Combat change
		add(curRenderedCombatNoteBools);
	...
	function addNoteUI():Void
	{
		...
		// Combat change
		check_isUnblockable = new FlxUICheckBox(noteTypeDropDown.x + noteTypeDropDown.width + 20, noteTypeDropDown.y, null, null, "Is Unblockable", 100);
		check_isUnblockable.checked = false;
		check_isUnblockable.name = 'check_isUnblockable';
		check_isUnblockable.callback = function()
		{
			if (curSelectedNote != null && curSelectedNote[1] > -1)
			{
				curSelectedNote[4] = check_isUnblockable.checked;
				updateGrid();
			}
		};
		tab_group_note.add(check_isUnblockable);

		check_isDeathNote = new FlxUICheckBox(check_isUnblockable.x, check_isUnblockable.y + check_isUnblockable.height + 20, null, null, "Is Kill Note", 100);
		check_isDeathNote.checked = false;
		check_isDeathNote.name = 'check_isDeathNote';
		check_isDeathNote.callback = function()
		{
			if (curSelectedNote != null && curSelectedNote[1] > -1)
			{
				curSelectedNote[5] = check_isDeathNote.checked;
				updateGrid();
			}
		};
		tab_group_note.add(check_isDeathNote);
		// End changes
	...
	function updateGrid():Void
	{
		...
		// Combat change
		curRenderedCombatNoteBools.clear();
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if (i[3] != null && note.noteType != null && note.noteType.length > 0)
			{
				...
				if (typeInt == null)
					theType = '?';
				// Combat change
				// Since there is a full note-type for a normal note, a 0 appears over them in the charter
				// The "normal" type is needed to avoid incorrect data being read from extra info attached to notes,
				// Thus the normal type is kept without looking visually different from vanilla
				if (typeInt == 0)
					theType = '';
			...
			// Combat change
			// The little orange-box indicates a note has the isUnblockable and isDeathNote bools flipped
			// The boxes are exempt from attack notes since they are innately visually distinct (i.e. they're already orange/red, respective),
			// With the one exception being that a death note gets the orange tick since the death note sprite overrules the unblockable sprite
			if (i[4] && (i[3] != 'attack' || i[5]))
			{
				var unblockableVis:FlxSprite = new FlxSprite(note.x, note.y).makeGraphic(8, 8, FlxColor.ORANGE);
				curRenderedCombatNoteBools.add(unblockableVis);
			}
			if (i[5] && i[3] != 'attack')
			{
				var deathVis:FlxSprite = new FlxSprite(note.x + 8, note.y).makeGraphic(8, 8, FlxColor.RED);
				curRenderedCombatNoteBools.add(deathVis);
			}
			// End of changes
		}
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		var daStrumTime = i[0];
		// Combat changes
		var daUnblockable = i[4];
		var daDeathNote = i[5];
		// End of changes
		var daSus:Dynamic = i[2];

		// Combat change
		// Add: daUnblockable, daDeathNote
		var note:Note = new Note(daStrumTime, daNoteInfo % 4, null, null, true, daUnblockable, daDeathNote);
		if (daSus != null)
		{ // Common note
