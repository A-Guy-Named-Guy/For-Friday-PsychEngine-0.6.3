...
enum abstract Action(String) to String from String
{
	...
	var GUP_P = "gup-press";
	var GLEFT_P = "gleft-press";
	var DDOWN_P = "ddown-press";
	var GRIGHT_P = "gright-press";
	var SPECIAL_P = 'special-press';
	
	var GUP_R = "gup-release";
	var GLEFT_R = "gleft-release";
	var DDOWN_R = "ddown-release";
	var GRIGHT_R = "gright-release";
	var SPECIAL_R = 'special-release';
...
class Controls extends FlxActionSet
{
	...
	// Combat changes
	var _gup = new FlxActionDigital(Action.GUP);
	var _gleft = new FlxActionDigital(Action.GLEFT);
	var _ddown = new FlxActionDigital(Action.DDOWN);
	var _gright = new FlxActionDigital(Action.GRIGHT);
	var _special = new FlxActionDigital(Action.SPECIAL);
	var _attack = new FlxActionDigital(Action.ATTACK);

	var _gupP = new FlxActionDigital(Action.GUP_P);
	var _gleftP = new FlxActionDigital(Action.GLEFT_P);
	var _ddownP = new FlxActionDigital(Action.DDOWN_P);
	var _grightP = new FlxActionDigital(Action.GRIGHT_P);
	var _specialP = new FlxActionDigital(Action.SPECIAL_P);
	var _attackP = new FlxActionDigital(Action.ATTACK_P);

	var _gupR = new FlxActionDigital(Action.GUP_R);
	var _gleftR = new FlxActionDigital(Action.GLEFT_R);
	var _ddownR = new FlxActionDigital(Action.DDOWN_R);
	var _grightR = new FlxActionDigital(Action.GRIGHT_R);
	var _specialR = new FlxActionDigital(Action.SPECIAL_R);
	...
	// Combat changes
	public var GUP(get, never):Bool;

	inline function get_GUP()
		return _gup.check();

	public var GLEFT(get, never):Bool;

	inline function get_GLEFT()
		return _gleft.check();

	public var DDOWN(get, never):Bool;

	inline function get_DDOWN()
		return _ddown.check();

	public var GRIGHT(get, never):Bool;

	inline function get_GRIGHT()
		return _gright.check();

	public var SPECIAL(get, never):Bool;

	inline function get_SPECIAL()
		return _special.check();

	public var GUP_P(get, never):Bool;

	inline function get_GUP_P()
		return _gupP.check();

	public var GLEFT_P(get, never):Bool;

	inline function get_GLEFT_P()
		return _gleftP.check();

	public var DDOWN_P(get, never):Bool;

	inline function get_DDOWN_P()
		return _ddownP.check();

	public var GRIGHT_P(get, never):Bool;

	inline function get_GRIGHT_P()
		return _grightP.check();

	public var SPECIAL_P(get, never):Bool;

	inline function get_SPECIAL_P()
		return _specialP.check();

	public var GUP_R(get, never):Bool;

	inline function get_GUP_R()
		return _gupR.check();

	public var GLEFT_R(get, never):Bool;

	inline function get_GLEFT_R()
		return _gleftR.check();

	public var DDOWN_R(get, never):Bool;

	inline function get_DDOWN_R()
		return _ddownR.check();

	public var GRIGHT_R(get, never):Bool;

	inline function get_GRIGHT_R()
		return _grightR.check();

	public var SPECIAL_R(get, never):Bool;

	inline function get_SPECIAL_R()
		return _specialR.check();
	...
	function forEachBound(control:Control, func:FlxActionDigital->FlxInputState->Void)
	{
		switch (control)
		{
			...
			case GUP:
				func(_gup, PRESSED);
				func(_gupP, JUST_PRESSED);
				func(_gupR, JUST_RELEASED);
			case GLEFT:
				func(_gleft, PRESSED);
				func(_gleftP, JUST_PRESSED);
				func(_gleftR, JUST_RELEASED);
			case DDOWN:
				func(_ddown, PRESSED);
				func(_ddownP, JUST_PRESSED);
				func(_ddownR, JUST_RELEASED);
			case GRIGHT:
				func(_gright, PRESSED);
				func(_grightP, JUST_PRESSED);
				func(_grightR, JUST_RELEASED);
			case SPECIAL:
				func(_special, PRESSED);
				func(_specialP, JUST_PRESSED);
				func(_specialR, JUST_RELEASED);
			case ATTACK:
	...
	public function setKeyboardScheme(scheme:KeyboardScheme, reset = true)
	{
		...
		switch (scheme)
		{
			case Solo:
				...
				// Combat changes
				inline bindKeys(Control.GLEFT, keysMap.get('guard_left'));
				inline bindKeys(Control.DDOWN, keysMap.get('guard_down'));
				inline bindKeys(Control.GUP, keysMap.get('guard_up'));
				inline bindKeys(Control.GRIGHT, keysMap.get('guard_right'));
				inline bindKeys(Control.ATTACK, keysMap.get('attack'));
				inline bindKeys(Control.SPECIAL, keysMap.get('special'));
				// End of changes
			...
			case Duo(true):
				...
				// Combat changes
				inline bindKeys(Control.GLEFT, [A]);
				inline bindKeys(Control.DDOWN, [S]);
				inline bindKeys(Control.GUP, [W]);
				inline bindKeys(Control.GRIGHT, [D]);
				inline bindKeys(Control.ATTACK, [CONTROL]);
				inline bindKeys(Control.SPECIAL, [SPACE]);
				// End of changes
			...
			case Duo(false):
				...
				// Combat changes
				inline bindKeys(Control.GLEFT, [FlxKey.LEFT]);
				inline bindKeys(Control.DDOWN, [FlxKey.DOWN]);
				inline bindKeys(Control.GUP, [FlxKey.UP]);
				inline bindKeys(Control.GRIGHT, [FlxKey.RIGHT]);
				inline bindKeys(Control.ATTACK, [CONTROL]);
				inline bindKeys(Control.SPECIAL, [SPACE]);
				// End of changes
	...
	public function addDefaultGamepad(id):Void
	{
		...
		addGamepadLiteral(id, [
			...
			// Combat changes
			// The letter-buttons have actions attached, so it's out as a valid note option
			// Basically the Y,A,X,B is deleted at the end
			...
			*/
			// Combat changes
			Control.GUP => [DPAD_UP, LEFT_STICK_DIGITAL_UP, RIGHT_STICK_DIGITAL_UP],
			Control.GLEFT => [DPAD_LEFT, LEFT_STICK_DIGITAL_LEFT, RIGHT_STICK_DIGITAL_LEFT],
			Control.DDOWN => [DPAD_DOWN, LEFT_STICK_DIGITAL_DOWN, RIGHT_STICK_DIGITAL_DOWN],
			Control.GRIGHT => [DPAD_RIGHT, LEFT_STICK_DIGITAL_RIGHT, RIGHT_STICK_DIGITAL_RIGHT],
			Control.SPECIAL => [A],
			Control.ATTACK => [X],
			// End of changes
			Control.PAUSE => [START],
