...
class Paths
{
	...
	public static function characterFileExists(curCharacter:String, canBeNull:Bool = false, characterFolder:Null<String> = null, fileType:String = '.json'):Bool
	{
		...
		if (FileSystem.exists(path))
			...
			path = getSharedPath(characterPath);

		if (FileSystem.exists(path))
		#else
		var path:String = getSharedPath(characterPath);
	...
	inline static public function songFile(song:String, fileName:String, ?modsAllowed:Bool = true):Sound
		return returnSound('${formatToSongPath(song)}/$fileName', 'songs', modsAllowed);
	...
	public static function returnSound(key:String, ?path:String, ?modsAllowed:Bool = true, ?beepOnNull:Bool = true)
	{
		...
		// Combat change
		var fileWav:String = getPath(Language.getFileTranslation(key) + '.wav', SOUND, path, modsAllowed);

		if (currentTrackedSounds.exists(fileWav))
			file = fileWav;
		else
		{
			#if sys
			if (FileSystem.exists(fileWav))
				file = fileWav;
			#else
			if (OpenFlAssets.exists(fileWav, SOUND))
				file = fileWav;
			#end
		}
		// End of change
