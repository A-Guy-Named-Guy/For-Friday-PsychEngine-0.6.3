...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	// Combat change
	var chunkBox:PsychUIBox;
	var chunkBoxPosition:FlxPoint = FlxPoint.get(40, 360);

	var chartNameArray:Array<String> = [];
	...
	override function create()
	{
		...
		// Combat change
		chunkBox = new PsychUIBox(chunkBoxPosition.x, chunkBoxPosition.y, 300, 150, ['Charts', 'Song File', 'General']);
		chunkBox.selectedName = 'Charts';
		chunkBox.scrollFactor.set();
		chunkBox.cameras = [camUI];
		add(chunkBox);
		// End of changes
		...
		// Combat change
		if (chartEditorSave.data.chunkBoxPosition != null && chartEditorSave.data.chunkBoxPosition.length > 1)
			chunkBox.setPosition(chartEditorSave.data.chunkBoxPosition[0], chartEditorSave.data.chunkBoxPosition[1]);
		// End of change
		...
		// Combat changes
		if (PlayState.SONG.chartArray == null || PlayState.SONG.chartArray == [])
		{
			PlayState.SONG.chartArray = [];

			PlayState.SONG.chartArray.push(newChartChunk('Inst'));
			PlayState.SONG.chartArray[0].voiceFileName = PlayState.SONG.song + '/Voices';
			saveToChartArray('Inst');
			PlayState.SONG.chartArray.push(newChartChunk('Intro'));
			PlayState.SONG.chartArray.push(newChartChunk('Outro'));
		}
		else
		{
			for (i in 0...PlayState.SONG.chartArray.length)
			{
				...
				if (PlayState.SONG.chartArray[i].songFileName == null)
					PlayState.SONG.chartArray[i].songFileName = PlayState.SONG.chartArray[i].chartName;
				if (PlayState.SONG.chartArray[i].voiceFileName == null)
					PlayState.SONG.chartArray[i].voiceFileName = PlayState.SONG.chartArray[i].chartName + 'Voices';
				if (PlayState.SONG.chartArray[i].chartNotes == null)
					PlayState.SONG.chartArray[i].chartNotes = [];
				if (PlayState.SONG.chartArray[i].chartEvents == null)
					PlayState.SONG.chartArray[i].chartEvents = [];
				if (PlayState.SONG.chartArray[i].chartBpm == null)
					PlayState.SONG.chartArray[i].chartBpm = PlayState.SONG.bpm;
				if (PlayState.SONG.chartArray[i].chartNeedsVoices == null)
					PlayState.SONG.chartArray[i].chartNeedsVoices = PlayState.SONG.needsVoices;
				if (PlayState.SONG.chartArray[i].chartSpeed == null)
					PlayState.SONG.chartArray[i].chartSpeed = PlayState.SONG.speed;
	...
	override function update(elapsed:Float)
	{
		...
		// Combat change
		// Adding overlap check for chunkBox
		// if (FlxG.mouse.justPressed && (FlxG.mouse.overlaps(mainBox.bg) || FlxG.mouse.overlaps(infoBox.bg)))
		if (FlxG.mouse.justPressed
			&& (FlxG.mouse.overlaps(mainBox.bg) || FlxG.mouse.overlaps(infoBox.bg) || FlxG.mouse.overlaps(chunkBox.bg)))
	...
	function loadMusic(?killAudio:Bool = false)
	{
		...
		// Combat change
		var currentChartChunk = getCurrentChartChunk();
		// End of change
		...
		if (killAudio)
		{
			...
			for (key => snd in Paths.currentTrackedSounds)
			{
				...
				// Combat change
				if (key.contains('/songs/${Paths.formatToSongPath(currentChartChunk.sourceFileName + '/' + currentChartChunk.songFileName)}/')
					&& snd != null)
				{
					sndsToKill.push(key);
					snd.close();
		...
		try
		{
			...
			var chartChunkSong:String = currentChartChunk.sourceFileName + '/' + currentChartChunk.songFileName;

			if (chartChunkSong != null && chartChunkSong != '')
				FlxG.sound.playMusic(Paths.returnSound(Paths.formatToSongPath(chartChunkSong), 'songs'), 0);
			else
				FlxG.sound.playMusic(Paths.inst(PlayState.SONG.song), 0);
			// End of change
		...
		if (PlayState.SONG.needsVoices)
		{
			try
			{
				...
				// Combat change
				if (currentChartChunk.voiceFileName != null && currentChartChunk.voiceFileName != '')
					playerVocals = Paths.returnSound(Paths.formatToSongPath(currentChartChunk.sourceFileName + '/' + currentChartChunk.voiceFileName
						+ '-Player'), 'songs');
				// End of change
				...
				// Combat change
				if (currentChartChunk.voiceFileName != null && currentChartChunk.voiceFileName != '')
					oppVocals = Paths.returnSound(Paths.formatToSongPath(currentChartChunk.sourceFileName + '/' + currentChartChunk.voiceFileName +
						'-Opponent'), 'songs');
				// End of change
	...
	function addViewTab()
	{
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Reset UI Boxes', function()
		{
			...
			// Combat change
			chunkBox.setPosition(chunkBoxPosition.x, chunkBoxPosition.y);
			// End of change
	...
	public function UIEvent(id:String, sender:Dynamic)
	{
		...
		switch (id)
		{
			...
			case PsychUIBox.DROP_EVENT:
				...
				// Combat change
				chartEditorSave.data.chunkBoxPosition = [chunkBox.x, chunkBox.y];
				// End of change
		}
	}
	...
	function openEditorPlayState()
	{
		...
		chunkBox.visible = upperBox.visible = mainBox.visible = infoBox.visible = false;
		// End of change
	}
	...
	override function closeSubState()
	{
		...
		chunkBox.visible = upperBox.visible = mainBox.visible = infoBox.visible = true;
		// End of changes
		upperBox.bg.visible = false;
		updateAudioVolume();
	}
	...
	// Combat change
	// Functions
	function updateChartNameArray()
	{
		chartNameArray = [];
		if (PlayState.SONG.chartArray == null)
			return;
		for (chart in PlayState.SONG.chartArray)
			chartNameArray.push(chart.chartName);
	}

	function writeFromChartArray(curChartName:String, switchingCharts:Bool = false):Void
	{
		var currentChartExists:Bool = false;

		for (i in 0...PlayState.SONG.chartArray.length)
			if (PlayState.SONG.chartArray[i].chartName == curChartName)
			{
		...
		{
			var chart:SwagChart = newChartChunk(curChartName);
			PlayState.SONG.chartArray.push(chart);
		}
		...
		for (i in 0...PlayState.SONG.chartArray.length)
			if (PlayState.SONG.chartArray[i].chartName == curChartName)
			{
				...
				var storedChart:SwagChart = PlayState.SONG.chartArray[i];

				PlayState.SONG.notes = [];

				for (ii in 0...storedChart.chartNotes.length)
				{
					var sec:SwagSection = newSection(storedChart.chartNotes[ii]);
					PlayState.SONG.notes.insert(sectionCount, sec);
					PlayState.SONG.notes[sectionCount] = sec;
				...
				for (ii in 0...storedChart.chartEvents.length)
					PlayState.SONG.events.push(storedChart.chartEvents[ii]);

				PlayState.SONG.bpm = storedChart.chartBpm;
				PlayState.SONG.needsVoices = storedChart.chartNeedsVoices;
				PlayState.SONG.speed = storedChart.chartSpeed;
				PlayState.SONG.offset = storedChart.chartOffset;

				if (switchingCharts)
				{
					songFileSourceInputText.text = storedChart.sourceFileName;
					chunkSongNameInputText.text = storedChart.songFileName;
					vocalsNameInputText.text = storedChart.voiceFileName;
				}

				bpmStepper.value = PlayState.SONG.bpm;
				allowVocalsCheckBox.checked = PlayState.SONG.needsVoices;
				scrollSpeedStepper.value = PlayState.SONG.speed;
	...
	function saveToChartArray(curChartName:String):Void
	{
		var currentChartExists:Bool = false;
		...
		for (i in 0...PlayState.SONG.chartArray.length)
			if (PlayState.SONG.chartArray[i].chartName == Song.retrieveNameInChartList(PlayState.SONG.activeChart, PlayState.SONG))
			{
				currentChartExists = true;
				break;
			}

		if (!currentChartExists && PlayState.SONG.notes.length > 0)
			PlayState.SONG.chartArray.push(newChartChunk(curChartName));
		...
		for (i in 0...PlayState.SONG.chartArray.length)
			if (PlayState.SONG.chartArray[i].chartName == curChartName)
			{
				...
				for (ii in 0...PlayState.SONG.notes.length)
				{
					var sec:SwagSection = newSection(PlayState.SONG.notes[ii]);
					PlayState.SONG.chartArray[i].chartNotes.insert(sectionCount, sec);
					PlayState.SONG.chartArray[i].chartNotes[sectionCount] = sec;
				...
				for (ii in 0...PlayState.SONG.events.length)
					PlayState.SONG.chartArray[i].chartEvents.push(PlayState.SONG.events[ii]);
	...
	function switchChart(chart:String, ?saveStartingChart:Bool = true):Void
	{
		updateChartData();
		if (saveStartingChart)
			saveToChartArray(PlayState.SONG.activeChart);
		PlayState.SONG.activeChart = chart;
		writeFromChartArray(PlayState.SONG.activeChart, true);
		prepareReload();

		updateChartNameArray();
	...
	{
		var currentChart:Null<SwagChart> = null;

		for (i in 0...PlayState.SONG.chartArray.length)
			if (PlayState.SONG.chartArray[i].chartName == PlayState.SONG.activeChart)
				currentChart = PlayState.SONG.chartArray[i];

		return currentChart;
	}

	var chartDropDown:PsychUIDropDownMenu;
	var newChartNameInputText:PsychUIInputText;
	var songFileInputText:PsychUIInputText;

	function addChartsTab()
	{
		var tab_group = chunkBox.getTab('Charts').menu;
		var objX = 10;
		var objY = 20;

		updateChartNameArray();
		chartDropDown = new PsychUIDropDownMenu(objX, objY, chartNameArray, function(id:Int, chart:String)
		{
			switchChart(chart);
		});
		chartDropDown.selectedLabel = 'Inst';

		objX += 140;

		newChartNameInputText = new PsychUIInputText(objX, objY, 120, '', 8);

		objX -= 7;

		var addChartButton:PsychUIButton = new PsychUIButton(objX, objY + 25, 'Add Chart', function()
		{
			var chartExists = false;

			for (i in 0...PlayState.SONG.chartArray.length)
				if (PlayState.SONG.chartArray[i].chartName == newChartNameInputText.text)
					chartExists = true;

			if (newChartNameInputText.text == '' || newChartNameInputText.text == null)
				showOutput('The new chart needs a name!', true);
			else if (!chartExists)
			{
				var newChartName:String = newChartNameInputText.text;

				PlayState.SONG.chartArray.push(newChartChunk(newChartName));
				switchChart(newChartName);
			}
			else
				showOutput('Chart name "${newChartNameInputText.text}" already exists!', true);
		}, 60);
		addChartButton.normalStyle.bgColor = FlxColor.GREEN;

		var removeChartButton:PsychUIButton = new PsychUIButton(objX + 65, objY + 25, 'Delete Chart', function()
		{
			var func:Void->Void = function()
			{
				resetSelectedNotes();
				...
				for (i in 0...PlayState.SONG.chartArray.length)
					if (PlayState.SONG.chartArray[i].chartName == PlayState.SONG.activeChart)
					{
						PlayState.SONG.chartArray[i] = null;
						PlayState.SONG.chartArray.remove(null);
					}
				switchChart('Inst', false);
			}

			var importantCharts:Array<String> = ['Inst', 'Intro', 'Outro'];
			if (importantCharts.contains(PlayState.SONG.activeChart))
				showOutput('Current chart "${PlayState.SONG.activeChart}" has special behaviors and shouldn\'t be deleted. Clear notes and/or adjust settings accordingly instead.',
					true);
			else
				openSubState(new Prompt('This chart\'s notes will be deleted.\nDelete current chart:\n"${PlayState.SONG.activeChart}"?', func));
		}, 70);
		removeChartButton.normalStyle.bgColor = FlxColor.RED;

		tab_group.add(new FlxText(chartDropDown.x, chartDropDown.y - 15, 120, 'Current Selected Chart:'));
		tab_group.add(new FlxText(newChartNameInputText.x, newChartNameInputText.y - 15, 180, 'New Chart Name:'));
		tab_group.add(chartDropDown);
		tab_group.add(newChartNameInputText);
		tab_group.add(addChartButton);
		tab_group.add(removeChartButton);
	}

	var songFileSourceInputText:PsychUIInputText;
	var chunkSongNameInputText:PsychUIInputText;
	var vocalsNameInputText:PsychUIInputText;

	function addChunkSongTab()
	{
		var tab_group = chunkBox.getTab('Song File').menu;
		var objX = 10;
		var objY = 20;

		var txt = new FlxText(objX, objY, 280, "Use the vanilla Song tab for the following:\nBPM, Scroll Speed, Offset, and Allow Vocals, ");
		txt.alignment = CENTER;
		tab_group.add(txt);

		objY += 40;

		var defaultChart = getCurrentChartChunk();

		songFileSourceInputText = new PsychUIInputText(objX, objY, 100, defaultChart.sourceFileName, 8);
		songFileSourceInputText.onChange = function(old:String, cur:String) getCurrentChartChunk().sourceFileName = cur;

		chunkSongNameInputText = new PsychUIInputText(objX + 120, objY, 100, defaultChart.songFileName, 8);
		chunkSongNameInputText.onChange = function(old:String, cur:String) getCurrentChartChunk().songFileName = cur;

		objY += 40;

		vocalsNameInputText = new PsychUIInputText(objX + 120, objY, 100, defaultChart.voiceFileName, 8);
		vocalsNameInputText.onChange = function(old:String, cur:String) getCurrentChartChunk().voiceFileName = cur;

		tab_group.add(new FlxText(songFileSourceInputText.x, songFileSourceInputText.y - 15, 160, 'File Source:'));
		tab_group.add(songFileSourceInputText);
		tab_group.add(new FlxText(chunkSongNameInputText.x, chunkSongNameInputText.y - 15, 160, 'Song File Name:'));
		tab_group.add(chunkSongNameInputText);
		tab_group.add(new FlxText(vocalsNameInputText.x, vocalsNameInputText.y - 15, 160, 'Voices File Name:'));
		tab_group.add(vocalsNameInputText);
	}

	var disableCombatCheckbox:PsychUICheckBox;
	var endSongOnDefeatCheckbox:PsychUICheckBox;
	var skipCountdownCheckbox:PsychUICheckBox;

	function addChunkGeneralTab()
	{
		var tab_group = chunkBox.getTab('General').menu;
		var objX = 10;
		var objY = 20;

		disableCombatCheckbox = new PsychUICheckBox(objX, objY, 'Disable Combat Mechanics', 80, function()
		{
			PlayState.SONG.disableCombat = disableCombatCheckbox.checked;
		});
		...
		objY += 40;
		skipCountdownCheckbox = new PsychUICheckBox(objX, objY, 'Skip Countdown', 80, function()
		{
			PlayState.SONG.skipIntroOnRestart = skipCountdownCheckbox.checked;
		});

		tab_group.add(disableCombatCheckbox);
		tab_group.add(endSongOnDefeatCheckbox);
