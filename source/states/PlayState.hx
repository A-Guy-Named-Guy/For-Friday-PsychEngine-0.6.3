...
// Combat change note
// This is just a reminder to make sure everything needed is imported
// The imports enforce pretty strict ordering so the comments cannot be placed where the new imports are
...
class PlayState extends MusicBeatState
{
	...
	public var flipBoyfriendAndDad:Bool = false;
	...
	public static var reverseByZ = function(Order:Int, Obj1:Dynamic, Obj2:Dynamic):Int
	{
		return FlxSort.byValues(FlxSort.DESCENDING, Obj1.zDepth, Obj2.zDepth);
	}
	...
	// Combat change
	// This is for fully implementing the character change event
	...
	override public function create()
	{
		...
		dad.curSpriteGroup = dadSpriteGroup;
		...
		//
		// Just to note: The purpose of boyfriendGroup/dadGroup seems to be for applying universal changes to all added boyfriends/dads
		// In other words it's support for changing characters through a lua event
		...
		boyfriend.curSpriteGroup = boyfriendSpriteGroup;
	...
	function startCharacterScripts(name:String)
	{
		...
		var scriptFile:String = 'characters/$name/$name.hx';
		if (!Paths.characterFileExists(name, false, name, '.hx'))
			scriptFile = 'characters/$name.hx';
		// End of changes
		#if MODS_ALLOWED
	...
	// Combat change
	// chartSong for chart chunks
	private function generateSong(chartSong:String):Void
	{
	...
	function set_health(value:Float):Float // You can alter how icon animations work here
	{
		...
		// Combat change
		// Added SONG.disableCombat check
		//
		// Since vanilla health is treated more like stamina
		// When combat is enabled, the icons animate according to combat health instead
		if (SONG.disableCombat)
		{
	...
	{
		// Combat change
		// Added combat health check for death
		// Added check for vanilla health death triggering only if character has deathByStamina
		// Added debugGodMode check
		// if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead && gameOverTimer == null)
		if ((((skipHealthCheck && instakillOnMiss) || health <= 0 && boyfriend.deathByStamina)
				...
				switch (charType)
				{
					case 0:
						if (boyfriend.curCharacter != value2)
						{
							...
							// Combat change
							// Boyfriend's health gets changed right after this
							...
							if (!SONG.disableCombat)
							{
								COMBAT.generateCombatBar('player', 'combatHealth');
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if (!note.hitCausesMiss) // Common notes
		{
			...
			if (!SONG.disableCombat)
			{
				if (boyfriend.combatHealth <= boyfriend.combatHealthMax)
	 ...
	 * @param section The SwagSection you're currently generating
	 ...
	 * @param ghostNotesCaught This transfers a var to update a sort of debug message in the initial note code
	 */
	function generateNotes(section:SwagSection, storageList:Array<Note>, ghostNotesCaught:Int = 0):Void
	{
		// Change notes
		// storageList is equivalent to the unspawnNotes array used in initial generation
		...
		for (i in 0...section.sectionNotes.length)
		{
			...
			{
				// CLEAR ANY POSSIBLE GHOST NOTES
				for (evilNote in storageList)
				{
					var matches:Bool = (noteColumn == evilNote.noteData && gottaHitNote == evilNote.mustPress && evilNote.noteType == noteType);
					if (matches && Math.abs(spawnTime - evilNote.strumTime) == 0.0)
					{
						evilNote.destroy();
						storageList.remove(evilNote);
						ghostNotesCaught++;
						// continue;
					}
				}
