...
class FunkinLua
{
	...
	public function new(scriptName:String)
	{
		...
		Lua_helper.add_callback(lua, "setCombatHealthMax", function(characterName:String, newHealthMax:Float, scaleHealth:Bool = true)
		{
			...
			characterBar.setBounds(0, character.combatHealthMax);
		});
		...
		Lua_helper.add_callback(lua, "setPostureMax", function(characterName:String, newPostureMax:Float, scalePosture:Bool = true)
		{
			...
			characterBar.setBounds(0, character.postureMax);
		});
		...
		{
			return PlayState.instance.healthBar.bounds.max;
		});
		Lua_helper.add_callback(lua, "setStaminaMax", function(newStaminaMax:Float, scaleStamina:Bool = true)
		{
			...
			var staminaMax:Float = PlayState.instance.healthBar.bounds.max;

			if (scaleStamina)
				PlayState.instance.health = stamina * newStaminaMax / staminaMax;

			PlayState.instance.healthBar.setBounds(0, newStaminaMax);
		});
		Lua_helper.add_callback(lua, "addStaminaMax", function(value:Float, addToStamina:Bool = true, scaleStamina:Bool = false)
		{
			var stamina:Float = PlayState.instance.health;
			var currentStaminaMax:Float = PlayState.instance.healthBar.bounds.max;
		...
		Lua_helper.add_callback(lua, "switchCombatJson", function(character:String, jsonName:String, cleanCharacter:Bool = true)
		{
			...
			var characterPath:String = 'characters/$characterName/$jsonName.json';
			if (!Paths.characterFileExists(jsonName, false, characterName, '.json'))
			{
				characterPath = 'characters/$jsonName.json';
				luaTrace('Json to switch not found at characters/$characterName/$jsonName.json,\nchecking characters/$jsonName.json');
			}
			// End of changes
			...
			{
				luaTrace('Error loading combat file of "$characterName": $e');
				return;
			}
		...
		Lua_helper.add_callback(lua, "cleanSlate", function(characterName:String)
		{
			cleanSlate(characterName);
		});
		...
		Lua_helper.add_callback(lua, "enemyAttack", function(attack:Null<AttackData> = null)
		{
			if (attack == null)
				return;
			PlayState.instance.COMBAT.startEnemyAttack(attack);
		});
		...
		Lua_helper.add_callback(lua, "togglePlayerControls", function(controlsActive:Null<Bool>)
		{
			...
			// So this is to make true enable controls and false disable them instead of the other way around
	...
	function getCharacterBar(characterName:String, barName:String):Bar
	{
		var characterBar:Bar = null;
	...
	function cleanSlate(characterName:String)
	{
		var character = getCharacter(characterName);

		character.combatHealth = character.combatHealthMax;
		character.posture = character.postureMax;
		character.actionTimer.cancel();
