...
typedef NoteParams =
{
	is_unblockable:Null<Bool>,
	is_death:Null<Bool>,
	attack_name:Null<String>
}
...
class Note extends FlxSprite
{
	...
	**/
		...
		if (noteData > -1 && noteType != value)
		{
			switch (value)
			{
				...
				case 'Attack Note' | 'attack':
					reloadNote('noteSkins/attackNOTE_asset');

					if (noteData == 1)
						isSpecialNote = true;
				case 'Attack Startup' | 'wind':
					reloadNote('noteSkins/windNOTE_asset');
				case 'windshort':
					reloadNote('noteSkins/windShortNOTE_asset');
			}
	...
	// Combat changes
	public function setNoteParams(params:NoteParams)
	{
		if (params.is_unblockable != null)
			isUnblockable = params.is_unblockable;
		if (params.is_death != null)
			isDeathNote = params.is_death;
		// if (params.attack_name != null)
		//	isUnblockable = params.is_unblockable;

		if (noteType == 'attack' || noteType == 'Attack Note')
		{
			if (isDeathNote)
